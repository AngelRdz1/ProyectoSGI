<div class="card" id="tablaPromedioTrimestral">
    <div class="fs-8 fs-xxl-6 py-5 px-5 text-gray-700">
        <div class="row">
            <form id="formPromedioTrimestral" action="" method="GET">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="row col-md-12 align-items-end">
                            ...
                            <div class="form-group col-md-2 mb-2">
                                ...
                                <option value="1" {{ old('trimestre', $request->trimestre) == 1 ? 'selected' : '' }}>Trimestre 1</option>
                                <option value="2" {{ old('trimestre', $request->trimestre) == 2 ? 'selected' : '' }}>Trimestre 2</option>
                                <option value="3" {{ old('trimestre', $request->trimestre) == 3 ? 'selected' : '' }}>Trimestre 3</option>
                            </select>
                            </div>
                            <!-- Boton-->
                            <div class="form-group col-md-2 mb-2">
                ...
                <tbody>
                    @foreach ($estudiantes as $estudiante)
                        <tr>
                            ...
                            @foreach ($headers as $header)
                                <td>{{ $matrizPromedioTrimestral[$estudiante->nie_estudiante][$header->nombre] }}</td>
                            @endforeach
                        </tr>
                    @endforeach
                </tbody>
...
<script>
        ...
        var table = $('#datatable').DataTable({
            ...
            buttons: [
                    ...
                    customize: function(xlsx) {
                        ...
                        $('row c[r^="A"]', sheet).each(function() {
                            if ($('is t', this).text().trim() === 'Reportes') {
                                $('is t', this).text('REPORTE PROMEDIO TRIMESTRAL\nCOLEGIO LUZ DE ISRAEL\nAÑO: ' + anio + '    GRADO: ' + grado + '    SECCION: ' + seccion + '    TRIMESTRE: ' + trimestre);
                            }
                        });
                    }
                },
                {
                    ...
                    title: grado + seccion + ' REPORTE PROMEDIO TRIMESTRAL',
                    customize: function(doc) {
                        doc.defaultStyle.fontSize = 11;

                        // Personalizar el título del documento PDF
                        doc.content[0].text = 'REPORTE PROMEDIO TRIMESTRAL\nCOLEGIO LUZ DE ISRAEL\nAÑO: ' + anio + '    GRADO: ' + grado + '    SECCION: ' + seccion + '    TRIMESTRE: ' + trimestre;
    ...
    $(document).ready(function() {
        $("#btnReporteExcel").on("click", function() {
            ...
            $.ajax({
                url: "{{ route('reportes.reportepromediotrimestral.excel') }}",
                type: 'GET',
            });
        });
        $("#btnReportePDF").on("click", function() {
            ...
            $.ajax({
                url: "{{ route('reportes.reportepromediotrimestral.pdf') }}",
                type: 'GET',
            });
        });
        ...
        $("#btnExtraer").on("click", function(event) {
            ...
            $.ajax({
                url: "{{ route('reportes.reportepromediotrimestral.index') }}",
                type: 'GET',
                data: {
                    anio: anio,
                    grado: grado,
                ...
                success: function(response) {
                    $("#tablaPromedioTrimestral").replaceWith(response);
                }
            });
        });
    });
